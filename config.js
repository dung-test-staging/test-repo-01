...
const env = envalid.cleanEnv(
  process.env,
  {
    ...
    CORE_API_URI: url({}),
    OPERARIUS_URI: url({ default: 'http://operarius:8080' }),
    ...
    AZURE_API_URL: url({ default: 'https://dev.azure.com' }),
    AZURE_OWN_USER_TOKEN: str({ default: '' }),
    ...
    REDIS_POLLING_MODE: bool({ default: false }),
    ...
    DAST_ENGINE_JOBS_MEM_LIMITS: str({ default: '4Gi' }),
    ...
    DAST_JOB_FOLDER: str({ default: '/opt/mount/consumables/' }),
    ...
    STOP_SCANNING_FEATURE: bool({ default: false }),
    DOCKER_NETWORK_NAME: str({ default: 'guardrails_network' }),
    GIT_PROVIDER_RATE_LIMIT_THRESHOLD: num({ default: 2000 }),
    USE_CWE: bool({ default: true }),
    UPDATE_COMMIT_MESSAGE_MAX_RETRY: num({ default: 3 }),
    UPDATE_COMMIT_MESSAGE_RETRY_INTERVAL_IN_SECOND: num({
      default: 5
    }),
    FEATURE_SECRET_TOKEN_VERIFICATION: bool({ default: false }),
    DEBUG_DAST_ENGINE: bool({ default: false })
  },
...
const constants = (environment => {
  const constantsByEnvironment = {
    ...
    sharedScanDataRedisTimeout: 10800 // 3 hours
  };
...
const engineResources = {
  ...
  'dast-owasp-zap': {
    requests: {
      cpu: env.DISABLE_ENGINE_JOBS_REQUESTS ? '1m' : '100m',
      memory: env.DISABLE_ENGINE_JOBS_REQUESTS ? '1Mi' : '1Gi'
    },
    limits: {
      cpu: env.ENGINE_JOBS_CPU_LIMITS,
      memory: env.DAST_ENGINE_JOBS_MEM_LIMITS
    }
  },
  // set double cpu limit for some golang engine to test cpu throttling improvement
  'go-gosec': {
    requests: {
